...
######################################### P A T H ############################################
complex_path = "/home/yeonji/Dropbox/myfolder_data/min_ahr_aligned/ahr_aligned_complex/"    # ahr/bbr different path
in_out_path = "/home/yeonji/Dropbox/myfolder_data/wbp_last/"
##############################################################################################

complex_files = sorted(glob.glob(complex_path + "pparg_ahr*pdb"))
...
for file in complex_files:
    ...
    if target:
        ...
        ### Input files path ###
        sim = "ahr"
        sim_spec = "ahr_eq"    # only ahr is different from sim
        # avg_file = in_out_path + sim + "_avg_l20/" + target + "_" + sim_spec + "_l20_average.pdb"   # f20/l20 different
        avg_file = "/gibbs/yeonji/DUDE/" + target.upper() + "/simulation/" + target + "_ahr_eq_l20_avg.pdb"

        if os.path.exists(avg_file):    # only include systems needed
            print(target)
            ### input files ###
            hs_summary_file = pd.read_csv(in_out_path + "ahr_eq/csv/" + target + "_" + sim_spec + "_hsa_summary.csv")
            clustercenter_file = md.load(in_out_path + "ahr_eq/cc/" + target + "_" + sim_spec + "_cc.pdb")
            mpo_file = in_out_path + "ahr_eq/mpo/" + target + "_" + sim_spec + "_unshifted_mpo.pdb"

            # hs_summary_file = pd.read_csv(in_out_path + "bbr_dist_2.4_hsa/SSTMap_HSA/ptn1_bbr_last_dist_2.4A_hsa_summary.csv")
            # clustercenter_file = md.load(in_out_path + "bbr_dist_2.4_hsa/SSTMap_HSA/clustercenterfile.pdb")
            # mpo_file = in_out_path + "bbr_dist_2.4_hsa/SSTMap_HSA/ptn1_bbr_dist_2.4A_unshifted_mpo.pdb"

            ### output files ###
            # close_output_file_name = in_out_path + "/match/fcnl_grp_close/" + target + "_" + sim_spec + "_match_data_close.csv"
            # all_output_file_name = in_out_path + "/match/fcnl_grp_all/" + target + "_" + sim_spec + "_match_data_all.csv"
            # pymol_session_file_name = in_out_path + "/pymol/" + target + "_" + sim_spec + "_vis.pse"

            close_output_file_name = in_out_path + "/result_edited_ligand/" + target + "_" + sim_spec + "_match_data_close.csv"
            all_output_file_name = in_out_path + "/result_edited_ligand/" + target + "_" + sim_spec + "_match_data_all.csv"
            pymol_session_file_name = in_out_path + "/result_edited_ligand/" + target + "_" + sim_spec + "_vis.pse"
            ...
            ### save the results ###
            data_table_closest.to_csv(close_output_file_name)
            data_table_all.to_csv(all_output_file_name)
            ...
            if hs_na_sele_str:
                pymol.cmd.select("na", "mpo and resi " + hs_na_sele_str)
                pymol.preset.ball_and_stick("na", mode=1)
                pymol.cmd.color("palecyan", "na and element o")
            ...
            pymol.cmd.load(avg_file, "avg")     # load average file
            pymol.cmd.select("avg_side_chains", "avg and resi " + pl_int_resi_sele_str)
            pymol.cmd.show("sticks", "avg_side_chains")
            pymol.cmd.set("stick_radius", 0.2, "avg")
            if sim == "ahr":
                avg_color = "orange"
            elif sim == "bbr":
                avg_color = "marin"
                ...
                avg_color = "green"
            pymol.cmd.color(avg_color, "avg and element c")
            ...
            pymol.cmd.save(pymol_session_file_name)
            pymol.cmd.delete("*")
